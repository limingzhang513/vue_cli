<template>
    <li>
        <label>
            ...
            <span>
                <!-- 使用@blur 触发失去焦点事件 $event可以获取当前标签对象-->
                <input type="text" v-show="todo.isedit" :value="todo.title" ref="updateValue" @blur="handelrOk(todo, $event)">
...
export default {
    ...
    methods: {
        ...
        updateTilteOk(todo){
            ...
            if(!this.$refs.updateValue.value.trim()){
                alert("输入不能为空")
                // setTimeout(()=>{
                //     this.$refs.updateValue.focus()  //加入等待任务才能触发获取焦点
                // }, 200)
        ...
        // 失去焦点回调，失去焦点事件
        handelrOk(todo, event){
            todo.isedit = false
            if(!event.target.value.trim()) return alert("输入不能为空") // 验证输入不能为空，当失去焦点触发函数，input框隐藏，数据不变
            this.$bus.$emit("updatetitle", todo.id, event.target.value)
        }
    },
}
</script>
